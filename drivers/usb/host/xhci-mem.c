...
static void xhci_free_segments_for_ring(struct xhci_hcd *xhci, struct list_head *seg_list)
{
	struct list_head *pos;
	struct list_head *tmp;
	struct xhci_segment *seg;

	list_for_each_safe(pos, tmp, seg_list) {
		seg = list_entry(pos, struct xhci_segment, list);
 ...
 */
static void xhci_link_trbs(struct xhci_segment *prev, struct xhci_segment *next, bool chain_links)
{
	union xhci_trb *trb;
	u32 val;
			    ...
			    struct list_head *new_list, unsigned int num_segs)
{
	...
	new_list_last = RING_LAST_SEG(new_list);
	tgt_list_last = RING_LAST_SEG(&ring->seg_list);
	...
	xhci_link_trbs(ring->enq_seg, list_next_entry(ring->enq_seg, list), chain_links);

	if (list_is_last(&new_list_last->list, &ring->seg_list)) {
		xhci_link_trbs(new_list_last, RING_FIRST_SEG(&ring->seg_list), chain_links);
		...
		xhci_link_trbs(new_list_last, list_next_entry(new_list_last, list), chain_links);
	}
}
...
void xhci_ring_free(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	...
	if (!list_empty(&ring->seg_list)) {
		...
		xhci_free_segments_for_ring(xhci, &ring->seg_list);
	}
			       ...
			       unsigned int cycle_state)
{
	...
	ring->enq_seg = RING_FIRST_SEG(&ring->seg_list);
					...
					gfp_t flags)
{
	...
	for (int num = 0; num < num_segs; num++) {
		...
		if (type != TYPE_EVENT)
			xhci_link_trbs(prev, next, chain_links);
		prev = next;
	}

	/* Link list end to start. */
	if (type != TYPE_EVENT)
		xhci_link_trbs(next, RING_FIRST_SEG(seg_list), chain_links);

	return 0;

free_segments:
	xhci_free_segments_for_ring(xhci, seg_list);
	return -ENOMEM;
}
		...
		enum xhci_ring_type type, unsigned int max_packet, gfp_t flags)
{
	...
	ret = xhci_alloc_segments_for_ring(xhci, &ring->seg_list, num_segs, cycle_state, type,
					   max_packet, flags);
	if (ret)
		goto fail;
	...
	if (type != TYPE_EVENT) {
		...
		RING_LAST_SEG(&ring->seg_list)->trbs[TRBS_PER_SEGMENT - 1].link.control |=
				...
				unsigned int num_new_segs, gfp_t flags)
{
	...
	ret = xhci_alloc_segments_for_ring(xhci, &new_list, num_new_segs, ring->cycle_state,
					   ring->type, ring->bounce_buf_len, flags);
	if (ret)
		return -ENOMEM;

	if (ring->type == TYPE_STREAM) {
		ret = xhci_update_stream_segment_mapping(ring->trb_address_map, ring, &new_list,
							 flags);
		if (ret)
			goto free_segments;
	}

	xhci_link_rings(xhci, ring, &new_list, num_new_segs);
...
free_segments:
	xhci_free_segments_for_ring(xhci, &new_list);
	return ret;
}
		...
		unsigned int max_packet, gfp_t mem_flags)
{
	...
	for (cur_stream = 1; cur_stream < num_streams; cur_stream++) {
		...
		addr = RING_FIRST_SEG(&cur_ring->seg_list)->dma |
...
int xhci_setup_addressable_virt_dev(struct xhci_hcd *xhci, struct usb_device *udev)
{
	...
	ep0_ctx->deq = cpu_to_le64(RING_FIRST_SEG(&dev->eps[0].ring->seg_list)->dma |
		...
		gfp_t mem_flags)
{
	...
	ep_ctx->deq = cpu_to_le64(RING_FIRST_SEG(&ep_ring->seg_list)->dma |
				  ep_ring->cycle_state);
...
struct xhci_interrupter *xhci_alloc_interrupter(struct xhci_hcd *xhci, unsigned int segs, gfp_t flags)
{
	...
	unsigned int num_segs, max_segs;
	int ret;
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	xhci_dbg_trace(xhci, trace_xhci_dbg_init, "First segment DMA is 0x%pad",
		       RING_FIRST_SEG(&xhci->cmd_ring->seg_list)->dma);

	/* Set the address in the Command Ring Control register */
	val_64 = xhci_read_64(xhci, &xhci->op_regs->cmd_ring);
	val_64 = (val_64 & (u64) CMD_RING_RSVD_BITS) |
		 (RING_FIRST_SEG(&xhci->cmd_ring->seg_list)->dma & (u64) ~CMD_RING_RSVD_BITS) |
