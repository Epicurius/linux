				     ...
				     u32 imod_interval)
{
	...
	imod_interval /= 250;
	if (imod_interval > U16_MAX)
		imod_interval = U16_MAX;

	irq_control = readl(&ir->ir_set->irq_control) & ~ER_IRQ_INTERVAL_MASK;
...
void xhci_set_cmd_ring_deq(struct xhci_hcd *xhci)
{
	...
	val_64 = (val_64 & (u64)CMD_RING_RSVD_BITS) | (deq_dma & (u64) ~CMD_RING_RSVD_BITS) |
...
static int xhci_reinit(struct xhci_hcd *xhci)
{
	...
	if (xhci->shared_hcd) {
		...
		ret = xhci_start(xhci->shared_hcd);
		if (ret)
			return ret;
