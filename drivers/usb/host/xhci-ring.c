...
void inc_deq(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	...
	if (ring->type == TYPE_EVENT) {
		...
		if (last_trb_on_ring(ring, ring->deq_seg, ring->dequeue))
			ring->cycle_state ^= 1;
		ring->deq_seg = _get_next_deq_seg(ring);
			...
			bool more_trbs_coming)
{
	...
	while (trb_is_link(next)) {
		 ...
		 */
		if (!xhci_chain_bit(xhci, ring->type)) {
		...
		ring->enq_seg = _get_next_enq_seg(ring);
					       ...
					       unsigned int num_trbs)
{
	...
	if (trb_is_link(ring->enqueue) && _get_next_enq_seg(ring)->trbs == ring->dequeue)
		return 0;
		...
		unsigned int ep_index)
{
	...
	if (trb_is_link(ep_ring->dequeue)) {
		ep_ring->deq_seg = _get_next_deq_seg(ep_ring);
		...
		u32 ep_state, unsigned int num_trbs, gfp_t mem_flags)
{
	...
	while (trb_is_link(ep_ring->enqueue)) {
		 ...
		 */
		if (!xhci_chain_bit(xhci, ep_ring->type))
		...
		ep_ring->enq_seg = _get_next_enq_seg(ep_ring);
